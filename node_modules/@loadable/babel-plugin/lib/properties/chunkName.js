...
const MATCH_LEFT_HYPHENS_REPLACE_REGEX = /^-/g; // https://github.com/webpack/webpack/blob/master/lib/Template.js
...
function readWebpackCommentValues(str) {
  try {
    const values = _vm.default.runInNewContext(`(function(){return {${str}};})()`);
...
function writeWebpackCommentValues(values) {
  try {
    const str = Object.keys(values).map(key => `${key}: ${JSON.stringify(values[key])}`).join(', ');
    return ` ${str} `;
  } catch (e) {
...
function getChunkNameComment(importArg) {
  ...
  return importArg.get('leadingComments').find(comment => comment.node.value.match(WEBPACK_CHUNK_NAME_REGEXP));
}
...
function moduleToChunk(str) {
  ...
  return str.replace(JS_PATH_REGEXP, '').replace(WEBPACK_PATH_NAME_NORMALIZE_REPLACE_REGEX, '-').replace(WEBPACK_MATCH_PADDED_HYPHENS_REPLACE_REGEX, '');
}
...
function chunkNameProperty({
  ...
  function transformQuasi(quasi, first, single) {
    return t.templateElement({
      ...
      cooked: single ? moduleToChunk(quasi.value.cooked) : replaceQuasi(quasi.value.cooked, first)
    }, quasi.tail);
  }
  ...
  function generateChunkNameNode(callPath) {
    ...
    if (importArg.isTemplateLiteral()) {
      return t.templateLiteral(importArg.node.quasis.map((quasi, index) => transformQuasi(quasi, index === 0, importArg.node.quasis.length === 1)), importArg.node.expressions);
    }
  ...
  function sanitizeChunkNameTemplateLiteral(node) {
    return t.callExpression(t.memberExpression(node, t.identifier('replace')), [t.regExpLiteral(WEBPACK_PATH_NAME_NORMALIZE_REPLACE_REGEX.source, 'g'), t.stringLiteral('-')]);
  }
